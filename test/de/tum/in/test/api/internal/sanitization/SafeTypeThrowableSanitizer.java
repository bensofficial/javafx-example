...
enum SafeTypeThrowableSanitizer implements de.tum.in.test.api.internal.sanitization.SpecificThrowableSanitizer {
    INSTANCE;
    ...
    public Throwable sanitize(Throwable t, MessageTransformer messageTransformer) {
        ...
        var info = de.tum.in.test.api.internal.sanitization.ThrowableInfo.of(type, ThrowableUtils.retrievePropertyValues(t,
        ...
        try {
            ...
            de.tum.in.test.api.internal.sanitization.SanitizationUtils.copyThrowableInfoSafe(info, newInstance);
            return newInstance;
        } catch (RuntimeException e) {
            LOG.warn("Failed to sanitize an exception of type {}.", type, e); //$NON-NLS-1$
            return de.tum.in.test.api.internal.sanitization.ArbitraryThrowableSanitizer.INSTANCE.sanitize(t, messageTransformer);
        }
    }
    ...
    private static class ThrowableCreatorWrapper implements ThrowableCreator {
        ...
        @Override
        public Throwable create(de.tum.in.test.api.internal.sanitization.ThrowableInfo throwableInfo) {
            if (throwableCreator == null)
    ...
    private static class HttpRetryExceptionCreator implements ThrowableCreator {
        ...
        @Override
        public Throwable create(de.tum.in.test.api.internal.sanitization.ThrowableInfo info) {
    ...
    private static class ComparisonFailureCreator implements ThrowableCreator {
        ...
        public Throwable create(de.tum.in.test.api.internal.sanitization.ThrowableInfo info) {
            ...
            var start = de.tum.in.test.api.internal.sanitization.SanitizationUtils.removeSuffixMatching(message, withoutMessage);
            if (start == null)
                message = ""; //$NON-NLS-1$
            else
                message = start.trim();
