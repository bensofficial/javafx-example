...
public final class AresSecurityConfigurationBuilder {
    ...
    private static final Path EXPECTED_MAVEN_POM_PATH = Path
            .of(System.getProperty(de.tum.in.test.api.security.AresSystemProperties.ARES_MAVEN_POM, "pom.xml")); //$NON-NLS-1$
    private static final Path EXPECTED_GRADLE_BUILD_PATH = Path
            .of(System.getProperty(de.tum.in.test.api.security.AresSystemProperties.ARES_GRADLE_BUILD, "build.gradle")); //$NON-NLS-1$
    ...
    static {
        ...
        IS_MAVEN = (StackWalker.getInstance().walk(sfs -> sfs.anyMatch(sf -> sf.getClassName().contains("maven"))) //$NON-NLS-1$
                ...
                && !Boolean.getBoolean(de.tum.in.test.api.security.AresSystemProperties.ARES_MAVEN_IGNORE);
