...
public final class ArtemisSecurityManager extends SecurityManager {
    ...
    static {
         ...
         */
        if (de.tum.in.test.api.security.SecurityConstants.MAIN_THREAD_GROUP == null) {
        ...
        if (Runtime.version().feature() == 17) {
            de.tum.in.test.api.security.SecurityConstants.SYSTEM_ERR
    ...
    private de.tum.in.test.api.security.AresSecurityConfiguration configuration;
    ...
    public static synchronized String install(de.tum.in.test.api.security.AresSecurityConfiguration configuration) {
    ...
    public void checkExit(int status) {
        try {
            ...
            if (enterPublicInterface())
                return;
            if (currentThreadGroup == de.tum.in.test.api.security.SecurityConstants.MAIN_THREAD_GROUP && getNonWhitelistedStackFrames().isEmpty()) {
    ...
    public void checkPackageDefinition(String pkg) {
        try {
            ...
            if (de.tum.in.test.api.security.SecurityConstants.STACK_WHITELIST.stream().anyMatch(pkg::startsWith))
    ...
    private boolean isPackageAccessForbidden(String pkg) {
        return de.tum.in.test.api.security.SecurityConstants.PACKAGE_USE_BLACKLIST.stream().anyMatch(pkg::startsWith)
    ...
    private boolean  isCallNotWhitelisted(String className, String methodName) {
        ...
        return de.tum.in.test.api.security.SecurityConstants.STACK_BLACKLIST.stream().anyMatch(call::startsWith)
                || (de.tum.in.test.api.security.SecurityConstants.STACK_WHITELIST.stream().noneMatch(call::startsWith)
